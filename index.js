...
for(let i = 0; i < roductsCart.length; i++){
   
    let lista2 = document.createElement('li')
    let descricao = document.createElement('p')
    let preco = document.createElement('span')
    lista2.className = 'classe'
    result = result + roductsCart[i].preco
        
    descricao.innerText = roductsCart[i].descricao
    preco.innerText = `R$ ${roductsCart[i].preco.toString().replace('.',',')}`
